...
import { Text, View, TextInput, Button, TouchableWithoutFeedback, Platform, Switch, Slider, Alert } from 'react-native';
...
const Component9: React.FC = () => {
    ...
    const handleCreateItem = async (itemToCreate: MuseumItemType) => {
        ...
        try {
            const response = await fetch('htpp://{server-address}:1337/museum-items',
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify(itemToCreate) // body data type match "Content-Type" header
                });
            ...
            // Here, set state to show that item has been successfully created. Meanwhile, check your
        } catch (error) {
